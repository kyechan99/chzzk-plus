...
export default function SettingButton() {
  const openSettingModal = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    chrome.storage.local.get([SETTING_MODAL], res => {
      const current = !!res[SETTING_MODAL];
      chrome.storage.local.set({
        [SETTING_MODAL]: !current,
      });
    });
  };

  return (
    <button onClick={openSettingModal} className={`czp-setting-btn `}>
      <svg
