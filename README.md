# Cheese Plus

<p align="center">
    <img src="./public/icon128.png" width=100/>
</p> 
<p align="center">
    <a href="https://chzzk.naver.com/">치지직</a> 확장 프로그램 
</p>
<p align="center">
    <img src="https://img.shields.io/github/package-json/v/kyechan99/chzzk-plus?style=for-the-badge"/>
    <img src="https://img.shields.io/github/license/kyechan99/chzzk-plus?style=for-the-badge"/>
    <a href="https://chromewebstore.google.com/detail/chzzk-plus/miampiopgfpnimmggagljgbpmjmjdjia">
 	    <img src="https://img.shields.io/badge/DOWNLOAD-ME-%2320c997?style=for-the-badge"/>
    </a>
</p>

[크롬 확장프로그램 설치](https://chromewebstore.google.com/detail/chzzk-plus/miampiopgfpnimmggagljgbpmjmjdjia)

## Feature

- 즐겨찾기 / 그룹 (팔로우 채널 중 우선순위 올리기)
- 생방송 실시간 미리보기
- 레이아웃 커스텀 구성하기
- 통나무 자동 수집
- 방송 시작 알림
- 방송 종료 알림
- 오디오 압축 (오디오 평준화 및 고음역 보호)
- 채팅 저장소 (자주 사용하는 채팅 저장하고 쉽게 복사하기)
- 특정 유저 채팅 고정
- 비도오 화면 필터
- 생방송 지연 시간 표시
- 생방송 빨리감기
- 시청자수 브라우저탭 표기
- 채팅 색상 모드 (커스텀 색상, 채팅 색상)
- 채팅 크기 조절
- 치즈 채팅 끄기
- 블라인드 채팅 끄기
- 구독 채팅 끄기
- 생방송 시작시 자동 새로고침
- 차단한 방송 완전 제거
- 팔로우 채널 자동 새로고침
- 화면 보호기
- 채팅 뱃지 제거거
- 단축키 기능
- 자동 넓은 화면
- 새 탭으로 열기
- 채팅 타임스탬프

## Preview

![0](/README/czp_1.png)
![1](/README/czp_2.png)
![2](/README/czp_3.png)
![3](/README/czp_4.png)
![4](/README/czp_5.png)

## 알림사항

치즈 플러스는 스트리밍 서비스 치지직(Chzzk)을 위한 비공식 오픈소스 확장 프로그램입니다.

본 확장 프로그램은 치지직 및 네이버와 어떠한 공식적인 연관이나 제휴 관계가 없습니다.

서비스 정책 및 플랫폼 구조를 준수하며, 저작권 및 비즈니스 모델(BM)을 침해할 수 있는 기능(예: 캡처, 녹화 등)은 제공하지 않습니다.

치지직 또는 네이버의 정책 변경이나 요청에 따라 일부 기능은 사전 고지 없이 제거될 수 있습니다.

확장 프로그램 사용으로 인해 발생하는 문제에 대해서는 개발자가 책임을 보장하지 않습니다.

치지직 서비스 자체의 업데이트로 인해 특정 기능이 일시적으로 동작하지 않을 수 있습니다.

해당 기능이 복구되기 전까지 비활성화하여 기다려주시면 감사드리겠습니다.

Cheese-Plus는 오픈소스로 개발되고 있습니다.
기능 제안 및 개발 기여 모두 적극적으로 받고있습니다.
